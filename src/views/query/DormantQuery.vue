<template>
  <div class="dormant-query">
    <div class="search-panel">
      <el-form :model="form" inline>
        <el-form-item label="最后入库起始日期"><el-date-picker v-model="form.startDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" /></el-form-item>
        <el-form-item label="最后入库截止日期"><el-date-picker v-model="form.endDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" /></el-form-item>
    ...
    <el-table :data="tableData" border stripe size="small" max-height="520">
      ...
      <el-table-column label="最后入库时间" width="120"><template #default="{ row }">{{ formatDate(row.inboundTime) }}</template></el-table-column>
      <el-table-column prop="dormantDays" label="呆滞天数" width="90">
        <template #default="{ row }"><span class="text-danger">{{ row.dormantDays }}天</span></template>
      </el-table-column>
...
const categories = MATERIAL_CATEGORIES.slice(0, 6)
...
async function loadData() {
  try {
    ...
    allData.value = invRes.data.list.filter(i => i.dormantDays > 0)
...
function doSearch() {
  ...
  data.sort((a, b) => b.dormantDays - a.dormantDays)
...
function doExport() {
  const headers = ['物料名称','物料编号','项目编号','物料类别','最后入库时间','呆滞天数','剩余库存','仓库','货位','所属人']
  const rows = tableData.value.map(i => [i.materialName, i.materialCode || '-', i.projectCode, i.materialCategory, formatDate(i.inboundTime), i.dormantDays + '天', i.remainingStock, i.warehouse, i.location, i.owner])
  exportToExcel('呆滞查询', headers, rows, '呆滞')
