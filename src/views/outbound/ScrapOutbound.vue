<template>
  <div class="scrap-outbound">
    ...
    <el-card shadow="never">
      <template #header>
        <span><el-icon><Search /></el-icon> 报废库存检索</span>
      </template>
      <el-row :gutter="16" class="filter-row">
        <el-col :span="8">
          <el-input v-model="keyword" placeholder="模糊查询：需报废的物料名称或编号" clearable @keyup.enter="search" />
        </el-col>
        <el-col :span="4">
      ...
      <el-table :data="inventoryList" stripe border size="small" max-height="400" v-loading="loading">
        ...
        <el-table-column label="报废数量" min-width="110" align="center" fixed="right">
          <template #default="{ $index }">
            <el-input-number v-model="qtyInputs[$index]" :min="1" :max="getAvailable(inventoryList[$index])" size="small" class="qty-input" />
          </template>
        </el-table-column>
        <el-table-column label="操作" min-width="60" align="center" fixed="right">
          <template #default="{ row, $index }">
            <el-button type="danger" size="small" circle @click="addToCart(row, $index)">
    ...
    <el-card shadow="never" class="cart-card">
      <template #header>
        <div class="card-header">
          ...
          <span class="count-text">
            共 {{ cart.reduce((s: number, c: any) => s + c.scrapQty, 0) }} 件
          </span>
        </div>
      </template>
      ...
      <el-table :data="cart" stripe border size="small" v-if="cart.length > 0">
        ...
        <el-table-column label="报废数量" min-width="110" align="center" class-name="highlight-qty">
          <template #default="{ row }">
            <el-input-number v-model="row.scrapQty" :min="1" :max="getAvailable(row)" size="small" class="qty-input" />
          </template>
        </el-table-column>
        <el-table-column prop="unit" label="单位" min-width="50" align="center" />
        <el-table-column prop="inboundTime" label="入库时间" min-width="130" show-overflow-tooltip>
        ...
        <el-table-column prop="inboundNo" label="入库单号" min-width="120" show-overflow-tooltip />
        <el-table-column prop="remainingStock" label="报废前余库" min-width="90" align="right" />
        <el-table-column prop="agingDays" label="账龄周期" min-width="80" align="right">
          <template #default="{ row }">{{ row.agingDays || 0 }}天</template>
        </el-table-column>
        <el-table-column prop="dormantDays" label="呆滞周期" min-width="80" align="right">
      ...
      <el-empty v-else description="请从上方检索区添加需要报废的物料" />
...
const form = reactive({ scrapOrderNo: '', approver: '待分配' })
...
function getAvailable(item: any): number {
  return Math.max(0, (item.remainingStock || 0) - (lockedMap.value[item.inboundNo] || 0))
}
...
async function loadData() {
  ...
  try {
    ...
    inventoryList.value = (invRes.data.list || []).filter((i: any) => getAvailable(i) > 0)
    qtyInputs.value = {}
...
function search() {
  ...
  loadData().then(() => {
    if (kw) {
      inventoryList.value = inventoryList.value.filter((i: any) =>
...
function addToCart(row: any, index: number) {
  const qty = qtyInputs.value[index] || 1
  const available = getAvailable(row)
  if (qty <= 0 || qty > available) { ElMessage.error('可用库存不足'); return }
  const existing = cart.value.find(c => c.inboundNo === row.inboundNo)
  if (existing) {
    if (existing.scrapQty + qty > available) { ElMessage.error(`合并失败：超出可用库存上限 (${available})`); return }
    existing.scrapQty += qty
  } else {
    cart.value.push({ ...row, scrapQty: qty })
  }
  ElMessage.success(`已添加 ${qty} ${row.unit || ''} [${row.materialName || row.materialCode || '未知物料'}] 到报废清单`)
}
